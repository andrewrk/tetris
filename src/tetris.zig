...
const DrawParticlePixelsFn = fn(callback_user_pointer: *u64, p: Particle) void;
const DrawRectanglePixelsFn = fn(callback_user_pointer: *u64, color: Vec4, mvp: Mat4x4) void;
const DrawTextPixelsFn = fn(callback_user_pointer: *u64, text: []const u8, left: i32, top: i32, size: f32) void;
...
pub const Tetris = struct {
    ...
    callback_user_pointer: *u64,
    drawParticlePixels: DrawParticlePixelsFn,
    drawRectanglePixels: DrawRectanglePixelsFn,
    drawTextPixels: DrawTextPixelsFn,
    ...
    pub fn draw(t: *Tetris, callback_user_pointer: *u64, drawParticlePixels: DrawParticlePixelsFn, drawRectanglePixels: DrawRectanglePixelsFn, drawTextPixels: DrawTextPixelsFn) void {
        t.callback_user_pointer = callback_user_pointer;
        t.drawParticlePixels = drawParticlePixels;
        t.drawRectanglePixels = drawRectanglePixels;
        t.drawTextPixels = drawTextPixels;
        t.fillRect(board_color, board_left, board_top, board_width, board_height);
        t.fillRect(board_color, next_piece_left, next_piece_top, next_piece_width, next_piece_height);
        t.fillRect(board_color, score_left, score_top, score_width, score_height);
        t.fillRect(board_color, level_display_left, level_display_top, level_display_width, level_display_height);
        t.fillRect(board_color, hold_piece_left, hold_piece_top, hold_piece_width, hold_piece_height);
...

        {
            ...
            drawTextPixels(
                ...
                score_text,
                score_left + score_width / 2 - @divExact(t.textPixelsWidth(score_text), 2),
        ...
        }
        {
            ...
            t.drawTextPixels(t.callback_user_pointer, score_text, score_left + score_width / 2 - @divExact(t.textPixelsWidth(score_text), 2), score_top + score_height / 2, 1.0);
        }
        {
            const text = "LEVEL:";
            t.drawTextPixels(t.callback_user_pointer, text, level_display_left + level_display_width / 2 - @divExact(t.textPixelsWidth(text), 2), level_display_top + margin_size, 1.0);
        }
        {
            var text_buf: [20]u8 = undefined;
            const text = bufPrint(text_buf[0..], "{}", t.level) catch unreachable;
            t.drawTextPixels(t.callback_user_pointer, text, level_display_left + level_display_width / 2 - @divExact(t.textPixelsWidth(text), 2), level_display_top + level_display_height / 2, 1.0);
        }
        {
            const text = "HOLD:";
            t.drawTextPixels(t.callback_user_pointer, text, hold_piece_left + hold_piece_width / 2 - @divExact(t.textPixelsWidth(text), 2), hold_piece_top + margin_size, 1.0);
        }

        for (t.falling_blocks) |maybe_particle| {
            if (maybe_particle) |particle| {
                t.drawFallingBlock(particle);
            }
        }

        for (t.particles) |maybe_particle| {
            if (maybe_particle) |particle| {
                t.drawParticlePixels(t.callback_user_pointer, particle);
            }
        }
    }
    ...
    fn fillRect(t: *Tetris, color: Vec4, x: f32, y: f32, w: f32, h: f32) void {
        ...
        t.drawRectanglePixels(t.callback_user_pointer, color, mvp);
    }
...
//      const model = mat4x4_identity.translateByVec(p.pos).rotate(p.angle, p.axis).scale(p.scale_w, p.scale_h, 0.0);
        ...
        t.drawRectanglePixels(t.callback_user_pointer, p.color, mvp);
    }

    fn drawCenteredText(t: *Tetris, text: []const u8) void {
        const draw_left = board_left + board_width / 2 - @divExact(t.textPixelsWidth(text), 2);
        const draw_top = @intCast(i32, board_top + board_height / 2 - font_char_height / 2);
        t.drawTextPixels(t.callback_user_pointer, text, draw_left, draw_top, 1.0);
    }
    ...
    fn drawPieceWithColor(t: *Tetris, piece: Piece, left: i32, top: i32, rot: usize, color: Vec4) void {
        for (piece.layout[rot]) |row, y| {
            for (row) |is_filled, x| {
                ...
                t.fillRect(color, abs_x, abs_y, cell_size, cell_size);
            }
        }
    }
        ...
        for (t.grid) |row, y| {
            ...
            if (all_filled) {
                for (t.grid[y]) |cell, x| {
                    ...
                    const center_x = @intToFloat(f32, board_left + x * cell_size) + @intToFloat(f32, cell_size) / 2.0;
                    const center_y = @intToFloat(f32, board_top + y * cell_size) + @intToFloat(f32, cell_size) / 2.0;
    ...
    inline fn textPixelsWidth(t: *Tetris, text: []const u8) i32 {
        return font_char_width * @intCast(i32, text.len);
    }
};
